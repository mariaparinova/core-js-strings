...
function removeFirstOccurrences(str, value) {
  const indexOfVal = str.indexOf(value);

  return indexOfVal === -1
    ? str
    : str.slice(0, indexOfVal) + str.slice(indexOfVal + value.length);
}
...
function removeLastOccurrences(str, value) {
  const lastIndexOfVal = str.lastIndexOf(value);

  return lastIndexOfVal === -1
    ? str
    : str.slice(0, lastIndexOfVal) + str.slice(lastIndexOfVal + value.length);
}
...
function countVowels(str) {
  ...
  let amountOfVowelsInStr = 0;

  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i].toLowerCase())) {
      amountOfVowelsInStr += 1;
    }
  }

  return amountOfVowelsInStr;
}
...
function isPalindrome(str) {
  ...
  const letters = str.match(/[a-z]/gi).join('');

  if (str.length < 2) {
    return true;
  }

  const middleOfLetters = Math.floor(letters.length / 2);
  let lastIndex = letters.length - 1;

  for (let i = 0; i <= middleOfLetters; i += 1) {
    if (letters.at(i).toLowerCase() !== letters.at(lastIndex).toLowerCase()) {
      return false;
    }
...
function reverseWords(str) {
  ...
  words.forEach((word) => {
    ...
    sentenceWithReversedWords += `${reversedWord} `;
  });

  return sentenceWithReversedWords.trim();
}
...
function encodeToRot13(str) {
  function getEncodedChar(char) {
    ...
    const mid = isUpperCase ? 'M'.charCodeAt(0) : 'm'.charCodeAt(0);
    const charCodeInput = char.charCodeAt(0);
    const charCodeOutput =
      charCodeInput <= mid ? charCodeInput + 13 : charCodeInput - 13;

    return String.fromCharCode(charCodeOutput);
  }

  let encodedString = '';

  for (let i = 0; i < str.length; i += 1) {
    encodedString += str[i].match(/[?!\s]/) ? str[i] : getEncodedChar(str[i]);
  }
...
function getCardId(value) {
  ...
  const suitOfValue = value.at(-1);

  return suits.indexOf(suitOfValue) * 13 + ranks.indexOf(rankOfValue);
}
